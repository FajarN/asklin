    ...
    <section class="section">
        <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item">Kategori Pembayaran</div>
            </div>
        </div>
    </section>
    <div class="row">
...
$(function () {  
    var table = $('#table').DataTable({
        ...
        searching: false,
        ajax: { 
            url: "{{ route('kategori_pembayaran.list') }}",
            data: function (d) {
                d.search = $('#search').val()
            }
        },
...
function edit(id){
    $.ajax({
        type:"POST",
        url: '{{ route('kategori_pembayaran.edit') }}',
        data: { id: id },
        dataType: 'json',
...
$('#form').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: '{{ route('kategori_pembayaran.store') }}',
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
...
function deleteu(id){
    if (confirm("Hapus kategori ini?") == true) {
        ...
        $.ajax({
            type:"POST",
            url: "{{ route('kategori_pembayaran.delete') }}",
            data: { id: id },
            dataType: 'json',
