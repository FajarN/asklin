...
class SuratPenomoranController extends Controller
{
    ...
    {
        $jenis = JenisSurat::where('status', '1')->get();

        return view('backend.surat_penomoran.index', compact('jenis'));
    }

    public function list(Request $request)
    {
        if ($request->ajax()) {
            $data = SuratPenomoran::latest()->get();
            return Datatables::of($data)
                ->filter(function ($instance) use ($request) {
                    if (!empty($request->get('search'))) {
                        $instance->collection = $instance->collection->filter(function ($data) use ($request) {
                            if (Str::contains(Str::lower($data['tahun']), Str::lower($request->get('search')))){
                                return true;
                            }
                            return false;
                        });
    ...
    {
        $id = $request->id;
        $data = SuratPenomoran::updateOrCreate(
            ['id' => $id],
            [
    ...
    {
        $data = SuratPenomoran::find($request->id);
    ...
    {
        $data = SuratPenomoran::where('id',$request->id)->delete();
        return Response()->json($data);
    }
}