    ...
    <section class="section">
        <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item">Konas Master Hotel</div>
            </div>
        </div>
    </section>
    <div class="row">
                ...
                <form id="form" class="form" action="javascript:void(0)">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label>Nama</label>
                            <input type="text" id="hotel" name="hotel" class="form-control form-control-sm">
                        </div>
                        <div class="form-group">
                            <label>Alamat</label>
...
$(function () {  
    var table = $('#table').DataTable({
        ...
        searching: false,
        ajax: { 
            url: "{{ route('konas_master_hotel.list') }}",
            data: function (d) {
                d.search = $('#search').val()
            }
        },
...
function edit(id){
    $.ajax({
        type:"POST",
        url: '{{ route('konas_master_hotel.edit') }}',
        data: { id: id },
        dataType: 'json',
...
$('#form').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: '{{ route('konas_master_hotel.store') }}',
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
...
function deleteu(id){
    if (confirm("Hapus hotel ini?") == true) {
        ...
        $.ajax({
            type:"POST",
            url: "{{ route('konas_master_hotel.delete') }}",
            data: { id: id },
            dataType: 'json',
